	...
	for ( i=0; i < 3; ++i ) { // for each potential source operand...
        if ( pI->src[i] != NULL ) {
            if( pI->src[i]->donecycle > sim_cycle ) {
				...
				//CASE1
				if(MD_OP_FLAGS(pI->src[i]->op)&F_LOAD){
					//CASE1.1:[THIS insn] is [a branch insn] and [src insn] is [a load insn]
					//[Due to the way of implementation] of this simulator, THIS insn is ALREADY at DECODE stage not matter what stage the src insn is currently at
					//src insn: load		F D X M W
					//this insn:branch		  F D D D X M W
					//I use Notepad++ for editing, if format gets messed up, the above two lines are lined up like this: W1<->D2(last D), 1 for src insn line, 2 for THIS insn line
					if((MD_OP_FLAGS(pI->op)&F_CTRL)||(MD_OP_FLAGS(pI->op)&F_UNCOND)||(MD_OP_FLAGS(pI->op)&F_INDIRJMP)||(MD_OP_FLAGS(pI->op)&F_CALL)||(MD_OP_FLAGS(pI->op)&F_COND)||(MD_OP_FLAGS(pI->op)&F_DIRJMP)){
						if((pI->src[i]->status!=DONE)){
						pI->stalled = 1;
						return;
						}
					}
					//CASE1.2:THIS insn is NOT branch and [src insn] is [a load insn]
					//if src is written by F_LOAD, then we need to wait until MEM stage of src insn before moving current insn to EXE stage
					//e.g.	ld r1,0(r2)		ld r1,0(r2)
					//		sd r3,0(r1)		add r5,r1,r3
					//src insn: load					F D X M W
					//THIS insn:NOT branch				  F D D X M W
					//the above two lines are lined up like this:  W1<->X2, 1 for src insn line, 2 for THIS insn line
					else{
						if((pI->src[i]->status!=MEMORY_STAGE_COMPLETED) && (pI->src[i]->status!=DONE)){
							pI->stalled = 1;
							return;
						}
					}
				...
				//Case2:[THIS insn] is [a branch insn] and [src insn] is [NOT a load insn]([src is a load insn] is [covered by CASE1.1]) 
				//src insn: NOT load		F D X M W
				//ThIS insn:branch			  F D D X M W
				//the above two lines are lined up like this:  W1<->X2, 1 for src insn line, 2 for THIS insn line
				if((MD_OP_FLAGS(pI->op)&F_CTRL)||(MD_OP_FLAGS(pI->op)&F_UNCOND)||(MD_OP_FLAGS(pI->op)&F_INDIRJMP)||(MD_OP_FLAGS(pI->op)&F_CALL)||(MD_OP_FLAGS(pI->op)&F_COND)||(MD_OP_FLAGS(pI->op)&F_DIRJMP)){
					if((pI->src[i]->status!=MEMORY_STAGE_COMPLETED) && (pI->src[i]->status!=DONE)){
							pI->stalled = 1;
